...
bench_dir=util.data_root + "lab_performance/list_compre_benchmarks_def/code/code/"
# for_list=['for e_'+str(i)+' in x_'+str(i)+':\n' for i in range(5)]
# if_else_list=['e_1 if e_'+str(i)+'%2 else ' for i in range(5)]
func_def_str="def func_a():\n"
if_main_str="if __name__ == '__main__':\n    func_a()"
for i_for,e_for in enumerate(for_list[:1]):
    for i_if,e_if in enumerate(if_list[:1]):
        for i_if_else,e_if_else in enumerate(if_else_list[:1]):
            for i_input,e_input in enumerate(input_list[:1]):
                ...
                code_comp_list=code_complicated_str.split("\n")
                code_complicated_str=func_def_str+"".join(["    "+e+"\n" for e in code_comp_list])+if_main_str
                print(code_complicated_str)
                # print("----------------------")
                save_path=bench_dir+"_".join([str(i_for+1),str(i_if),str(i_if_else),e_input])+'_func.py'
