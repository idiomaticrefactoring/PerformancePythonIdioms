...
from call_star import performance_replace_content_by_ast_call_star

import refactor_call_star
...
def save_one_file_time_list(file_name, code_list):
    for ind, (arg_list, new_tree) in enumerate(code_list):
        old_tree = arg_list[0]
        ...
        #     continue
        if 1:
            old_content, new_content, flag_same = performance_replace_content_by_ast_call_star.replace_content_var_unpack_call_star_insert_time_simple(
                file_path, old_tree, new_tree,call_node )
        ...
        if 1:  # not os.path.exists(time_simp_bench_dir + file_name):
            old_content, new_content, flag_same = performance_replace_content_by_ast_call_star.replace_content_var_unpack_call_star_simple(
                file_path, old_tree, new_tree,call_node)
            # print("new_content: ")
...
if __name__ == '__main__':
    ...
    log_time_complicate_bench_dir = util.data_root_mv + "lab_performance/call_star_benchmarks_some/log/log_time_compli_code_prefined_cpu_new_improve/"
    ...
    log_time_simp_bench_dir = util.data_root_mv + "lab_performance/call_star_benchmarks_some/log/log_time_simp_code_prefined_cpu_new_improve/"
    ...
    time_complicate_bench_dir = util.data_root_mv + "lab_performance/call_star_benchmarks_some/code/time_complicated_code_prefined_cpu_new_improve/"
    time_simp_bench_dir = util.data_root_mv + "lab_performance/call_star_benchmarks_some/code/time_simple_code_prefined_cpu_new_improve/"

    # util.mkdirs(time_complicate_bench_dir)
    # util.mkdirs(time_simp_bench_dir)
    bench_time_info_dir=util.data_root_mv + "lab_performance/call_star_benchmarks_some/bench_time_info_dir_prefined_cpu_new_improve/"
    ...
    for ind,file_name in enumerate(dir_list[offset:]):#9001750
        ...
        # if ind>10:
        #     break
        if file_name!="0_Endflag_10_num_0_lower_1_step_2_subscript_func.py":#"0_Endflag_29_num_1_lower_1_step_2_subscript.py":#"6_ass_swap_func.py":
            continue
        # if os.path.exists(bench_time_info_dir + file_name[:-3] + ".pkl"):
        ...
        if '.py' not in file_name:
            continue
        if "1_Endfla" in file_name:
            code_list = refactor_call_star.refactor_call_star(bench_dir + file_name,1)
        else:
            code_list = refactor_call_star.refactor_call_star(bench_dir + file_name)
