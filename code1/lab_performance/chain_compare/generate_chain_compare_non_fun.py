...
for ind,(e1,e2) in enumerate(comb_1):
    ...
    print("code2_list,len(code2_list): ",code2_list,len(code2_list))
    ...
    code_var = var_str + "\n    " + code
    code_complicated_str = func_def_str + if_main_str + code_var+"\n    print('code is finished')"
    save_path = bench_dir + code + '.py'
    print(save_path)
    count += 1
...
for ind,(e1,e2,e3) in enumerate(comb_2):
    ...
    count += 1
    util.save_file_path(save_path, code_complicated_str)
    # code_list = refactor_chain_compare.refactor_chain_compare(save_path)
    # print(code_list)
    # print(">>>>>>: ",[(ast.unparse(code),ast.unparse(new_code)) for code,new_code in code_list])
    # util.save_file_path(save_path, code_complicated_str)
    print(code_complicated_str)
    # break
# '''
