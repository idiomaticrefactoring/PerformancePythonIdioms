...
def save_one_file_time_list(file_name, code_list):
    for ind, (old_tree, new_tree) in enumerate(code_list):
        ...
        #     continue
        if 1:
            old_content, new_content, flag_same = performance_replace_content_by_ast_chain_compare.replace_content_chain_compar_insert_time_simple(
        ...
        if 1:  # not os.path.exists(time_simp_bench_dir + file_name):
            old_content, new_content, flag_same = performance_replace_content_by_ast_chain_compare.replace_content_chain_compar_simple(
                file_path, old_tree, new_tree)
            # print("new_content: ")
        ...
        # print("all_time_list: ", all_time_list[0],all_pythonic_time_list[0])
...
if __name__ == '__main__':
    ...
    log_time_complicate_bench_dir = util.data_root_mv + "lab_performance/chain_compare_benchmarks_new_3/log/log_time_chain_compare_compli_code_prefined_cpu_new/"
    ...
    log_time_simp_bench_dir = util.data_root_mv + "lab_performance/chain_compare_benchmarks_new_3/log/log_time_chain_compare_simp_code_prefined_cpu_new/"
    ...
    time_complicate_bench_dir = util.data_root_mv + "lab_performance/chain_compare_benchmarks_new_3/code/time_complicated_code_prefined_cpu_new/"
    time_simp_bench_dir = util.data_root_mv + "lab_performance/chain_compare_benchmarks_new_3/code/time_simple_code_prefined_cpu_new/"

    # util.mkdirs(time_complicate_bench_dir)
    # util.mkdirs(time_simp_bench_dir)
    bench_time_info_dir=util.data_root_mv + "lab_performance/chain_compare_benchmarks_new_3/bench_time_info_dir_prefined_cpu_new/"
    ...
    # for file_name in dir_list[:end]:#9001750
    for file_name in dir_list[:end]:  # 9001750

        # if file_name!="5_>=_==_==_<_not in_func.py":#"0_<=_==_==_>_not in.py":
        #     continue
        # if os.path.exists(bench_time_info_dir + file_name[:-3] + ".pkl"):
        ...
        #     print("code: ",ast.unparse(code[0]),ast.unparse(code[1]))
        # print(code_list)
        if len(code_list) > 1:
...
#4_in_is not_is not_is not_is not.py
#3_in_in_is not_is not.py
# 4_in_in_in_in.py