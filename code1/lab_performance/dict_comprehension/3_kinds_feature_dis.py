...
def get_features_add_interval(save_code_info_dir_add_performance_change,save_code_info_dir_add_performance_change_remove_outlier):
    ...
    for ind, file_name in enumerate(dict_perf_change["file_html"]):
        ...
        dict_pd["num_ele"].append(int(lab_code_info.num_add_ele))
        num_list = file_name_no_suffix.split("_")
        num_loop, num_if, num_if_else, num_ele, *other = num_list
        # dict_pd["num_loop"].append(num_loop)
        # dict_pd["num_if"].append(num_if)
        # dict_pd["num_if_else"].append(num_if_else)
...
if __name__ == '__main__':
    ...
    dir_csv_feature= util.data_root_mv + "lab_performance/dict_compre_benchmarks/csv/"
    csv_feature_file_name_corr= util.data_root_mv + "lab_performance/dict_compre_benchmarks/csv/train_data_dict_compre.csv"
    dataMain = pd.DataFrame(data=dict_pd)
    dataMain.to_csv(csv_feature_file_name_corr, index=False)

    # df_sample = dataMain.sample(n=310, random_state=2022)
