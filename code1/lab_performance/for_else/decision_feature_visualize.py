...
# print(df)'regression', 'unchange', 'improve' 'regression':0,
df['kind'] = df['kind'].map({
                             'unchange':1,
                             'improve':2,
                             np.nan:'NY'},
...
feature_names = ["node_kind","branch_through_break",'has_else',
              'context',"num_ele"]
...
# tree.plot_tree(clf)
fn=feature_names
cn=['unchange', 'improve']#'regression',
...
plt.savefig("performance_rq1_for_else.pdf", format="pdf", bbox_inches="tight",pad_inches=0)
