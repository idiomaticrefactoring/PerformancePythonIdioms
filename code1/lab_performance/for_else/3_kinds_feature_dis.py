...
def get_features_add_interval(save_code_info_dir_add_performance_change,save_code_info_dir_add_performance_change_remove_outlier):
    ...
    dict_pd = {"file_html":[],"code_str":[],"RCIW":[],"perf_change_right":[],"perf_change_left":[],"perf_change":[],"kind":[],
               "node_kind":[],"branch_through_break":[],'has_else': [],"num_ele":[],
              'context': []}
    ...
    for ind, file_name in enumerate(dict_perf_change["file_html"]):
        ...
        dict_pd["num_ele"].append(int(file_name_no_suffix.split("_")[0]))
        if "for" in file_name:
            dict_pd["node_kind"].append(0)
        else:
            dict_pd["node_kind"].append(1)
        if "1_break" in file_name:
            dict_pd["branch_through_break"].append(1)

        else:
            dict_pd["branch_through_break"].append(0)
        if "1_else" in file_name:
            dict_pd["has_else"].append(1)
        else:
            dict_pd["has_else"].append(0)
...
if __name__ == '__main__':
    ...
    csv_feature_file_name_corr= util.data_root_mv + "lab_performance/for_else_benchmarks/csv/train_data_loop_else.csv"
    dataMain = pd.DataFrame(data=dict_pd)
