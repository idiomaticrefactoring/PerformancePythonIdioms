...
from multi_assign_time import performance_replace_content_by_ast_multi_assign

import refactor_multi_ass
from chain_compare_time import performance_replace_content_by_ast_chain_compare
...
def save_one_file_time_list(file_name, code_list):
    for ind, (old_tree, new_tree) in enumerate(code_list):
        ...
        #     continue
        if 1:
            old_content, new_content, flag_same = performance_replace_content_by_ast_multi_assign.replace_content_multi_ass_insert_time_simple(
        ...
        if 1:  # not os.path.exists(time_simp_bench_dir + file_name):
            old_content, new_content, flag_same = performance_replace_content_by_ast_multi_assign.replace_content_multi_ass_simple(
                file_path, old_tree, new_tree)
            # print("new_content: ")
...
if __name__ == '__main__':
    ...
    log_time_complicate_bench_dir = util.data_root + "lab_performance/multi_ass_benchmarks_some_again/log/log_time_chain_compare_compli_code_prefined_cpu_new/"
    ...
    log_time_simp_bench_dir = util.data_root + "lab_performance/multi_ass_benchmarks_some_again/log/log_time_chain_compare_simp_code_prefined_cpu_new/"
    ...
    time_complicate_bench_dir = util.data_root + "lab_performance/multi_ass_benchmarks_some_again/code/time_complicated_code_prefined_cpu_new/"
    time_simp_bench_dir = util.data_root + "lab_performance/multi_ass_benchmarks_some_again/code/time_simple_code_prefined_cpu_new/"

    # util.mkdirs(time_complicate_bench_dir)
    # util.mkdirs(time_simp_bench_dir)
    bench_time_info_dir=util.data_root + "lab_performance/multi_ass_benchmarks_some_again/bench_time_info_dir_prefined_cpu_new/"
    # util.mkdirs(bench_time_info_dir)
    bench_dir = util.data_root + "lab_performance/multi_ass_benchmarks/code/code/"
    ...
    file_name_list=["8_ass_swap_func.py",
"7_ass_swap_func.py","10_ass_swap_func.py"]
    file_name_list=[

        '8_ass_var_func.py','8_ass_swap_func.py','4_ass_var_func.py','9_ass_swap_func.py'
    ]
    count_remain=0
    for file_name in dir_list[:]:#9001750
        ...
        if '.py' not in file_name:
            continue
        code_list = refactor_multi_ass.refactor_multi_ass(bench_dir + file_name)
