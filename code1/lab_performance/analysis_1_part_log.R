...
alias( lm(log(perf_change) ~ log(num_for+1) + log(num_ele+1) + num_if + num_if_else
           , data=data1) )

vif_test <- lm(log(perf_change) ~  log(num_for+1) + log(num_ele+1) + num_if + num_if_else
                , data=data1)
vif(vif_test)

g <- gam(log(perf_change) ~   log(num_for+1) + log(num_ele+1) + num_if + num_if_else, data=data1,
         method = 'REML')
...
g <- gam(log(perf_change) ~ s(log(num_for+1), k = 4)+s(num_if, k = 5)+s(num_if_else, k = 5)+s(log(num_ele+1)), data=data1,
         method = 'REML')
summary(g)
