...
def get_features_add_interval(save_code_info_dir_add_performance_change,save_code_info_dir_add_performance_change_remove_outlier):
    ...
    dict_pd = {"file_html":[], "code_str":[],"RCIW":[],"perf_change_right":[],"perf_change_left":[],"perf_change":[],
     ...
     "is_total_data":[],'context': [],"size_data":[],"ratio_subs_unpack":[]}#"kind":[],
    ...
    for ind, file_name in enumerate(dict_perf_change["file_html"]):
        if "2_dim" not in file_name:
            continue
        print("this is true: ",file_name)
...
if __name__ == '__main__':
    ...
    dir_csv_feature= util.data_root + "lab_performance/for_multi_targets_benchmarks/csv/"
    csv_feature_file_name_corr= util.data_root + "lab_performance/for_multi_targets_benchmarks/csv/train_data_for_multi_targets_new_noDim.csv"
    dataMain = pd.DataFrame(data=dict_pd)
    dataMain.to_csv(csv_feature_file_name_corr, index=False)
    print(len(dataMain["file_html"]))
    df_sample = dataMain.sample(n=356, random_state=2022, ignore_index=True)
    get_reason(df_sample)
    df_sample.to_csv(dir_csv_feature + "sample_for_multi_tar.csv", index=False)
