...
bench_dir=util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new_paper_final/code/code/"
...
for i in range(input_list_ele_num):
    ...
    for dim_i in range(input_dimen):
        for unpack_i in range(input_unpack_sub_ele_num):
            ...
            if i==0:
                input_code = f"input_seq=[i for i in range(0)]\n"
            else:
...
bench_dir_sample=util.data_root_mv + "lab_performance/for_mult_tar_benchmarks_paper_sample_new_2/code/code_new/"
from random import sample
random.seed(2022)
sample_file_name_list=sample(file_name_list,2400)
            ...
            for subscript_i in range(1,subscript_num+1):
                for num_unpack in range(unpack_i+1):
                    ...
                    # subscr_code="\n".join(all_e_list)
                        ...
                        # print(f">>>{flag_need_star}flag_need_star;{10**i} input; {dim_i+2} input dim; {unpack_i+1} unpack_i;{subscript_i} subscript_i"
                        #       f">>>>>>>code_complicated_str: \n",code_complicated_str)
                        file_name=f"{flag_need_star}_flag_{10**i}_input_{dim_i+2}_dim_{unpack_i+1}_unpack_{subscript_i}_subscript_{num_unpack+1}_uniqueUnpack.py"
                        if file_name[:-3] + "_func.py" in sample_file_name_list:
