...
from for_multi_targets import performance_replace_content_by_ast_for_multi_targets

import refactor_for_multi_tar
...
def save_one_file_time_list(file_name, code_list,invocations,iterations):
    for ind, (old_tree, new_tree) in enumerate(code_list):
        ...
        #     continue
        if 1:
            old_content, new_content, flag_same = performance_replace_content_by_ast_for_multi_targets.replace_content_var_unpack_for_target_insert_time_simple(
        ...
        if 1:  # not os.path.exists(time_simp_bench_dir + file_name):
            old_content, new_content, flag_same = performance_replace_content_by_ast_for_multi_targets.replace_content_var_unpack_for_target_simple(
                file_path, old_tree, new_tree)
            # print("new_content: ")
...
if __name__ == '__main__':
    ...
    log_time_complicate_bench_dir = util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new/log/log_time_chain_compare_compli_code_prefined_cpu_new/"
    ...
    log_time_simp_bench_dir = util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new/log/log_time_chain_compare_simp_code_prefined_cpu_new/"
    ...
    time_complicate_bench_dir = util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new/code/time_complicated_code_prefined_cpu_new/"
    time_simp_bench_dir = util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new/code/time_simple_code_prefined_cpu_new/"

    # util.mkdirs(time_complicate_bench_dir)
    # util.mkdirs(time_simp_bench_dir)
    bench_time_info_dir=util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new/bench_time_info_dir_prefined_cpu_new/"

    # util.mkdirs(bench_time_info_dir)
    bench_dir = util.data_root + "lab_performance/for_multi_targets_benchmarks_2_new/code/code/"
    bench_dir = util.data_root_mv + "lab_performance/for_multi_targets_benchmarks_2_new/code/code/"
    ...
    for ind,file_name in enumerate(dir_list[offset:]):#9001750
        # if "1_flag_100000_input_3_dim_4_unpack_21_subscript_func.py"!=file_name:
        #     continue
        # if "_100000_input" not in file_name:
        #     continue
         # if ind>10:
        #     break
        if "_1000000_input" in file_name:
            invocations = 3
            iterations = 6
        elif "_100000_input" in file_name:
            invocations = 15
            iterations = 13
        elif "_10000_input" in file_name:
            invocations = 30
            iterations = 20
        else:
            invocations = 50
        ...
        if '.py' not in file_name:
            continue
        code_list = refactor_for_multi_tar.refactor_for_multi_target(bench_dir + file_name)
