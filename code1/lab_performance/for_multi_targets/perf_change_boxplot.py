...
if __name__ == '__main__':
    ...
    save_code_info_dir_add_performance_change_remove_outlier = \
        util.data_root + "lab_performance/for_multi_targets_benchmarks_2_new/prefined_cpus_remain_code_new_add_perf_change_remove_oulier_factor_3/50/"
    ...
    save_code_info_dir_add_performance_change = \
        util.data_root + "lab_performance/for_multi_targets_benchmarks_2_new/prefined_cpus_remain_code_new_add_perf_change/50/"
    save_code_info_dir_add_performance_change = \
        util.data_root + "lab_performance/for_multi_targets_benchmarks_2_new/prefined_cpus_remain_code_new_add_perf_change/50/"
    ...
    dict_pd_new=dict()
    code_str_list=dict_pd["code_str"]
    for key in dict_pd:
        dict_pd_new[key]=[]
        for i,ele in enumerate(dict_pd[key]):
            if "*" in code_str_list[i]:
                dict_pd_new[key].append(ele)
    dataMain = pd.DataFrame(data=dict_pd_new)

    dataMain.to_csv(csv_perf_change_dir + "two_configure_perf_change_ci_star.csv", index=False)
