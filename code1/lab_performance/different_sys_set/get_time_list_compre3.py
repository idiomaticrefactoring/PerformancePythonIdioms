...
if __name__ == '__main__':
    ...
    log_time_complicate_bench_dir = util.data_root + "lab_performance/list_compre_benchmarks/different_sys_set/log/log_time_list_compreh_compli_code/"
    log_time_simp_bench_dir = util.data_root + "lab_performance/list_compre_benchmarks/log/different_sys_set/log_time_list_compreh_simp_code/"
    util.mkdirs(log_time_complicate_bench_dir)
    util.mkdirs(log_time_simp_bench_dir)
    time_complicate_bench_dir = util.data_root + "lab_performance/list_compre_benchmarks/different_sys_set/code/time_complicated_code/"
    time_simp_bench_dir = util.data_root + "lab_performance/list_compre_benchmarks/different_sys_set/code/time_simple_code/"

    util.mkdirs(time_complicate_bench_dir)
    util.mkdirs(time_simp_bench_dir)
    bench_time_info_dir=util.data_root + "lab_performance/list_compre_benchmarks/different_sys_set/code_time_info/"
    util.mkdirs(bench_time_info_dir)
    bench_dir = util.data_root + "lab_performance/list_compre_benchmarks/code/code/"#code/
    total_start_time=time.time()
    for file_name in os.listdir(bench_dir)[:4]:
        # if file_name!="2_4_3_[1].py":#"1_1_0_list(range(1,11))*10.py":
        #     continue
        if file_name=="2_3_0_list(range(1,11))*1000.py":
            continue
        # if file_name!="1_1_0_list(range(1,11))*10.py":#"2_4_3_[1].py":#"4_0_0_[1].py":#"1_1_0_list(range(1,11))*10.py":
        #     continue
        ...
        for ind, (for_node, assign_node, remove_ass_flag, new_tree) in enumerate(code_list):
            ...
            old_content, new_content, flag_same = performance_replace_content_by_ast_time_percounter.replace_file_content_for_compre_3_category_insert_time(
            file_path, for_node,
            assign_node,
            ...
            for invo in range(invocations):
                output=lab_performance_util.sequence_run(time_complicate_bench_dir,file_name,invo=invo,log_dir=log_time_complicate_bench_dir+file_name[:-3]+"/")
