...
for ind,empty in enumerate(empty_list_opposite):
    ...
    for ind_is_true,equal_flag in enumerate(branch_true_false):
        for node in node_list:
            for is_true in [0, 1]:
                ...
                # print(code_complicated_str)
                file_name = f"{empty}_empty_{equal_flag}_equal_flag_{node}_node_{flag_is_true}_isTrue.py"
                if file_name in file_name_list:
                    print("what is wrong: ",file_name)
        ...
        # break
    # break
print("total code: ",count,len(file_name_list),len(set(file_name_list)))
