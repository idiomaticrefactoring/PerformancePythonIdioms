...
def get_features_add_interval(save_code_info_dir_add_performance_change,save_code_info_dir_add_performance_change_remove_outlier):
    ...
    for file_name in sorted(os.listdir(save_code_info_dir_add_performance_change)):
        ...
        try:
            ...
            new_file_name=str_file_name+str(int(file_name_no_suffix.split("_")[-1])+156)
            lab_code_info_add_isTrue = util.load_pkl(isTrue_code_info_dir, new_file_name)

        except:
            print(">>>> the file cannot load: ", save_code_info_dir_add_performance_change, file_name_no_suffix,new_file_name)
            continue
        try:
        ...
        for ind_tm, test_me in enumerate(perf_info_dict):
            ...
            for instance in perf_info_dict[test_me]:
                ...
                dict_pd['num_cmpop'].append(cmpop_1+cmpop_2+cmpop_3+cmpop_4+cmpop_5+cmpop_6+cmpop_7+cmpop_8+cmpop_9+cmpop_10)
    ...
    util.save_json(file_html_dir,"all_file_html_chain_compare",file_html_list)
...
if __name__ == '__main__':
    ...
    isTrue_code_info_dir= util.data_root + "performance/chain_compare_add_isTrue/chain_compare_iter_invoca/"
