...
if __name__ == '__main__':
    ...
    print("len of error: ",len(case_is_true_list_filter))
    pro_time_start = time.time()
    save_code_info_dir=util.data_root+"performance/chain_compare/chain_compare_iter_invoca/"
    error_code_dir = util.data_root + "performance/chain_compare/erro_record_dir/"
    ...
    for ind_code,code in enumerate(code_info_list[:]):
        ...
        # if file_html!="https://github.com/nccgroup/ScoutSuite/tree/master/ScoutSuite/core/conditions.py":#"https://github.com/kellyjonbrazil/jc/tree/master/jc/parsers/csv.py":
        #     continue
        ...
        if "and " in ast.unparse(new_tree):
            ...
            new_old_tree,new_new_tree=code.get_chain_compare_complicted_code_pythonic_code()
        ...
        if flag_same:
            case_is_true_list_filter.append([ind_code,"complicated code",file_html,cl, me_name, total_name])
            continue
    ...
    util.save_pkl(util.data_root +error_code_dir, "error_code_no_change_info_list", case_is_true_list_filter)
    '''
