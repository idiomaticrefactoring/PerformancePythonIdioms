...
import performance_replace_content_by_ast_chain_compare
        ...
        for test_html, each_rela_path, cl, me in test_me_inf_list:
            ...
            if not is_own_config:
                ...
                run_test_result_new = configure_pro_envir_util.run_test_file_capture_output_each_fun(test_html,
                                                                                                     ...
                                                                                                     iterations=iterations)
            print(
                f">>>>>>>>>>{(test_html, each_rela_path, cl, me)} {fun_list} run_test_result_new: ")  # , , run_test_result_newrun_test_result_new)
...
if __name__ == '__main__':
    ...
    save_code_info_dir = util.data_root + "performance/chain_compare/chain_compare_iter_invoca/"
    save_code_info_dir = util.data_root + "performance/chain_compare_add_isTrue/chain_compare_iter_invoca/"

    time_out_dir = util.data_root + "performance/chain_compare/time_out_record_dir/"

    file_dir = util.data_root + "format_code_info/"
    file_name = "chain_compare_complete"
    code_time_list = []
    ...
    file_html_dir=util.data_root_mv + "performance/chain_compare/"
    file_html_list=util.load_json(file_html_dir,"all_file_html_chain_compare")
    ...
    for ind_code, code in enumerate(code_info_list[:]):
        ...
        if "and " in ast.unparse(new_tree):
            ...
            # print("old_code: ", old_tree.lineno,old_tree, ast.unparse(old_tree),"\nnew_code: ", new_tree,ast.unparse(new_tree))
            old_tree, new_tree = code.get_chain_compare_complicted_code_pythonic_code()
            # print(" new_old_code: ", new_old_tree.lineno,new_old_tree, ast.unparse(new_old_tree),"\nnew_new_code: ", new_new_tree,ast.unparse(new_new_tree))
        else:
            print(f"existed >>>>>>>>>>>>>>>> existed total_name of {ind_code} code: ", file_html, cl, me_name,
                  total_name)
            # continue
            # pass
        ...
        # '''
        # old_content, new_content, flag_same = performance_replace_content_by_ast_chain_compare.replace_content_chain_compar_insert_time(
        #     old_path, old_tree, new_tree, me_name)

        old_content, new_content, flag_same = performance_replace_content_by_ast_chain_compare.replace_content_chain_compar_insert_time_addidTrue(
            old_path, old_tree, new_tree, me_name)
        # print(old_content)
        # print("***********************")
        # print(new_content)
        ...
        old_content, new_content, flag_same = performance_replace_content_by_ast_chain_compare.replace_content_chain_compar(
            old_path, old_tree, new_tree, me_name)
        if flag_same:
