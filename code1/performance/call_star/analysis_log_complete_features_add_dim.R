...
alias( lm(log(perf_change) ~ log(num_subscript) + log(is_value_const+1)  +log(is_step_1+1)+log(dim_subscript)
           , data=data1) )#+ is_upper_len + is_lower_0

vif_test <- lm(log(perf_change) ~ log(num_subscript) + log(is_value_const+1)  +log(is_step_1+1)+log(dim_subscript)
                , data=data1)#+ is_upper_len  + is_lower_0
vif(vif_test)

g <- gam(log(perf_change) ~   log(num_subscript) + log(is_value_const+1)  +log(is_step_1+1)+log(dim_subscript), data=data1,
         method = 'REML')# + is_upper_len  + is_lower_0
...
# plot_smooth(g,
#             transform = exp)
g <- gam(log(perf_change) ~    s(log(num_subscript),k=3) + log(is_value_const+1)  +log(is_step_1+1)+log(dim_subscript), data=data1,
         method = 'REML')#+ is_upper_len  + is_lower_0
summary(g)
