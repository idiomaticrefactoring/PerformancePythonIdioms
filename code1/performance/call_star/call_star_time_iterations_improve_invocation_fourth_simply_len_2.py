...
if __name__ == '__main__':
    ...
    file_html_list = util.load_json(file_html_dir, "all_file_html_call_star")
    save_code_info_dir = util.data_root_mv + "performance/a_call_star_single_3_simplify_len_again/a_call_star_iter_invoca_2/"
    error_code_dir = util.data_root_mv + "performance/a_call_star_single_3_simplify_len_again/erro_record_dir_2/"
    time_out_dir = util.data_root_mv + "performance/a_call_star_single_3_simplify_len_again/time_out_record_dir_2/"
    save_code_info_dir = util.data_root_mv + "performance/a_call_star_single_3_simplify_len_again_2/a_call_star_iter_invoca_2/"
    error_code_dir = util.data_root_mv + "performance/a_call_star_single_3_simplify_len_again_2/erro_record_dir_2/"
    time_out_dir = util.data_root_mv + "performance/a_call_star_single_3_simplify_len_again_2/time_out_record_dir_2/"
    save_code_info_dir = util.data_root_mv + "performance/a_call_star_single_4_simplify_len_again_2/a_call_star_iter_invoca_2/"
    error_code_dir = util.data_root_mv + "performance/a_call_star_single_4_simplify_len_again_2/erro_record_dir_2/"
    time_out_dir = util.data_root_mv + "performance/a_call_star_single_4_simplify_len_again_2/time_out_record_dir_2/"

    file_dir=util.data_root+"format_code_info/"
    save_code_info_dir_num = util.data_root_mv + "performance/a_call_star_single_2_add_size/a_call_star_iter_invoca_2/"
    ...
    for ind_code,code in enumerate(code_info_list[offset:end]):
        ...
        if file_html not in file_html_list:
            continue
        if file_html!="https://github.com/rthalley/dnspython/tree/master/dns/ipv6.py":
            continue
        # if file_html!="https://github.com/rthalley/dnspython/tree/master/dns/ipv6.py":
        #     continue
        ...
        # num_ele_offset_file_name_no_suffix = total_name + "_" + str(ind_code + num_ele_offset)
        # print(file_name_no_suffix,num_ele_offset_file_name_no_suffix)
        str_file_name = "_".join(file_name_no_suffix.split("_")[:-1]) + "_"
        new_file_name = str_file_name + str(int(file_name_no_suffix.split("_")[-1]) + 55)
        try:
            lab_code_info_add_num_ele = util.load_pkl(save_code_info_dir_num, new_file_name)
        except:
            continue
        ...
        for key in lab_code_info_add_num_ele.num_ele_list:
            len_star = int(lab_code_info_add_num_ele.num_ele_list[key][0][0])
            break
        ...
        # old_content, new_content, flag_same = performance_replace_content_by_ast_call_star.replace_content_var_unpack_call_star_simply_len(
        #     old_path, old_tree, new_tree, call_node, me_name, len_star)
        ...
        old_content, new_content, flag_same = performance_replace_content_by_ast_call_star.replace_content_var_unpack_call_star_simply_len(
            old_path, old_tree, new_tree, call_node, me_name, len_star)
        # old_content, new_content, flag_same = performance_replace_content_by_ast_call_star.replace_content_var_unpack_call_star_simply_len(replace_content_var_unpack_call_star_insert_time
        #     old_path, old_tree, new_tree,call_node, me_name)
        if flag_same:
