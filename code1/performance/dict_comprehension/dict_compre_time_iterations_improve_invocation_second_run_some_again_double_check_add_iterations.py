        ...
        for test_html, each_rela_path, cl, me in test_me_inf_list:
            ...
            if not is_own_config:
                ...
                run_test_result_new = configure_pro_envir_util.run_test_file_capture_output_each_fun(test_html,
                                                                                                     ...
                                                                                                     iterations=iterations)
            print(
                f">>>>>>>>>>{(test_html, each_rela_path, cl, me)} {fun_list} run_test_result_new: ")  # , run_test_result_new)#, run_test_result_new)
...
if __name__ == '__main__':
    ...
    save_code_info_dir = util.data_root_mv + "performance/a_dict_comprehension_some_again_double_check_add_iterations/dict_comprehension_iter_invoca_2/"
    time_out_dir = util.data_root_mv + "performance/a_dict_comprehension_some_again_double_check_add_iterations/time_out_record_dir2_/"
    ...
    error_code_dir = util.data_root_mv + "performance/a_dict_comprehension_some_again_double_check_add_iterations/erro_record_dir_2/"
    ...
    # threshold = 1#10
    # iterations = 2#15
    prefine_cpus ="taskset -c 4,5 " # "taskset -c 4,5 "
    offset = 34
    end = 67
    '''
...
total_time_list_info_dict:  dict_keys([('https://github.com/cloud-custodian/cloud-custodian/tree/master/tests/test_provider.py', 'tests.test_provider', 'ProviderTest', 'test_import_resource_classes')])>>>>>>>>>it does not have total_time_list_info_dict_merge
 ...
 total_time_list_info_dict:  dict_keys([]) https://github.com/mne-tools/mne-python/tree/master/mne/cov.py [['https://github.com/mne-tools/mne-python/tree/master/mne/tests/test_proj.py', 'mne.tests.test_proj', '', 'test_bad_proj'], ['https://github.com/mne-tools/mne-python/tree/master/mne/tests/test_cov.py', 'mne.tests.test_cov', '', 'test_whiten_evoked'], ['https://github.com/mne-tools/mne-python/tree/master/mne/tests/test_cov.py', 'mne.tests.test_cov', '', 'test_cov_order'], ['https://github.com/mne-tools/mne-python/tree/master/mne/tests/test_cov.py', 'mne.tests.test_cov', '', 'test_regularize_cov'], ['https://github.com/mne-tools/mne-python/tree/master/mne/simulation/tests/test_evoked.py', 'mne.simulation.tests.test_evoked', '', 'test_rank_deficiency']]
    ...
    for ind_code, code in enumerate(code_info_list[:]):
        ...
        if file_html!="https://github.com/pytoolz/toolz/tree/master/toolz/itertoolz.py":
            continue
        ...
        print(f">>>>>>>>>>>>>>>> total_name of {ind_code} code: ", file_html, cl, me_name, total_name, remove_ass_flag,
              code.own_config)
        flag_total_test_html = dict()
        flag_total_test_html_instances = dict()
        ...
        if flag_same:
            ...
            case_is_true_list_filter.append(
                [ind_code, ind_code + offset, "pythonic code", file_html, cl, me_name, total_name])
            continue
        ...
        print(len(all_invocations_time_list))  # ,flag_total_test_html,flag_total_test_html_instances)
        print(
            len(all_invocations_time_list_pythonic))  # , flag_total_test_html_pythonic, flag_total_test_html_instances_pythonic)
        ...
        print(">>>>>>>>>>>>>save successfully! ", save_code_info_dir + code_info_file_name,
              one_code_time_end - one_code_time_start)
        # break
    ...
    util.save_pkl(time_out_dir, "time_out_code_info_1", time_out_list_filter)
    util.save_pkl(error_code_dir, "error_code_is_true_info_1", case_is_true_list_filter)
    pro_time_end = time.time()
    print("total running time of the program: ", pro_time_end - pro_time_start)
    # '''
