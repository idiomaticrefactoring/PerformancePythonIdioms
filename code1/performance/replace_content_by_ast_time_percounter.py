...
class Rewrite_chain_compare_insert_time(ast.NodeTransformer):
    ...
    def generic_visit(self, node):
        ...
        return node
def replace_file_content_for_compre_3_category(repo_name,file_html,for_node,assign_stmt,new_code,remove_ass_flag=0,pro_path="python_star_2000repo/"):
    real_file_html = file_html.replace("//", "/")
    rela_path = "/".join(real_file_html.split("/")[6:])
    file_path = "".join([util.data_root, pro_path, repo_name, "/", rela_path])
...
def replace_file_content_for_compre_3_category_insert_time(repo_name, file_html, for_node, assign_stmt, new_code,
                                                   remove_ass_flag=0):
        real_file_html = file_html.replace("//", "/")
        rela_path = "/".join(real_file_html.split("/")[6:])
