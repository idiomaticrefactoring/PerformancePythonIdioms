    ...
    def generic_visit(self, node):
        if hasattr(node, "body"):
            ...
            for ind,child in enumerate(node.body):
                ...
                if 1:
                # for ind, child in enumerate(node.body):
                #     print("**********child:\n",ast.unparse(child),child.lineno,self.old_node.lineno,"\n",ast.unparse(self.old_node))
                    if ast.unparse(child) == ast.unparse(self.ass) and self.ass.lineno == child.lineno:
                        # print(">>>>come here: ", ast.unparse(child))
                        # node.body.remove(child)
                        ass_re=child
                    ...
                    if child.lineno == self.old_node.lineno and isinstance(child, ast.For) and ast.unparse(
                        ...
                        local='locals().keys()'
                        print_locals_stmt = f"print('locals: ',{local})"
                        for e in ast.walk(ast.parse(print_locals_stmt)):
                            if isinstance(e, ast.stmt):
                                node.body.insert(ind + 1, e)
                                break
                        var=ast.unparse(self.new_node).split("=")[0]
                        print_len_stmt = f"print('len: ',len({var}))"
                        for e in ast.walk(ast.parse(print_len_stmt)):
                        ...
                        # print(">>>>>>names: ",names,set(names))
                        # names = sorted({node.id for node in ast.walk(child) if isinstance(node, ast.Name)})
                        for name in set(names):

                            name_stmt=f"if count_zj:\n    try:\n        print('''\ntype: {name}: ''',type({name}))\n    except:\n        pass"
                            name_stmt_node=ast.parse(name_stmt)
                            for e in ast.walk(ast.parse(name_stmt_node)):
                                if isinstance(e, ast.stmt):
                                    node.body[ind].body.insert(1, e)
                                    break

                        ass_stmt_own_count = "count_zj=0"
                        for e in ast.walk(ast.parse(ass_stmt_own_count)):
                            if isinstance(e, ast.stmt):
                                node.body[ind].body.insert(len(node.body[ind].body), e)
                                break
                        ass_stmt_own_count = "count_zj=1"
                        for e in ast.walk(ast.parse(ass_stmt_own_count)):
                            if isinstance(e, ast.stmt):
                                node.body.insert(ind, e)
                                break
                        break
                    bia = old_len - len(node.body)
                    # print("bias: ", bia)
