...
if __name__ == '__main__':
    ...
    save_code_info_dir=util.data_root+"performance/a_multi_assign/a_multi_assign_iter_invoca_update_some/"
    ...
    print("len: ",len(file_html_list_filter),len(set(file_html_list_filter)))
    ...
    for ind_code,code in enumerate(code_info_list[:]):
        ...
        if file_html not in file_html_list_filter:
            continue
        count_invalid+=1
        #'''
        ...
        # print("old_tree, new_tree: ", new_tree,old_tree[0].lineno)
        # code.own_config=0
        ...
        old_content, new_content, flag_same = performance_replace_content_by_ast_multi_assign.replace_content_multi_ass_insert_time(
        ...
        old_content, new_content, flag_same = performance_replace_content_by_ast_multi_assign.replace_content_multi_ass(
            old_path, old_tree, new_tree, me_name)
        if flag_same:
            case_is_true_list_filter.append([ind_code,ind_code + offset, "pythonic code", file_html, cl, me_name, total_name, total_name + "_" + str(ind_code + offset)])
            continue
    ...
    util.save_pkl(error_code_dir, "error_code_is_true_info_update_some", case_is_true_list_filter)
    util.save_pkl(time_out_dir, "time_out_code_info_update_some", time_out_list_filter)
    #'''
    print(count_invalid)
