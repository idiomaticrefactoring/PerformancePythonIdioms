...
class Rewrite_multiple_ass(ast.NodeTransformer):
    def __init__(self,old_node,new_node):
        ...
        for node in ast.walk(ast.parse(new_node)):
            if isinstance(node,ast.stmt):
                self.new_node = node
                print("the new tree is ",ast.parse(node))
                break
        # self.new_node =new_node
    ...
    def remove_node(self, cur_node):
        ...
        if beg != -1:
            ...
            cur_node.insert(insert_index, self.new_node)
            for rem_node in remove_list:
                cur_node.remove(rem_node)
            insert_index = cur_node.index(self.new_node)
            cur_node.insert(insert_index + 1, self.print_total_stmt_pythonic)
    ...
    def generic_visit(self, node):
        ...
        return node
class Rewrite_multiple_ass_swap(ast.NodeTransformer):
    def __init__(self,old_node,new_node):
        self.old_node =old_node
        self.new_node=new_node
        # for node in ast.walk(ast.parse(new_node)):
    ...
    def remove_node(self, cur_node):
        ...
        for ind, child in enumerate(cur_node):
            ...
            if not isinstance(child, (ast.Assign, ast.AnnAssign)):
                ...
                continue
            # print("child: ", ast.unparse(child), child.lineno)
            if child.lineno==self.new_node.lineno and ast.unparse(child)==ast.unparse(self.new_node):
                insert_index=ind
                cur_node.insert(insert_index + 1, self.print_total_stmt_pythonic)
                cur_node.insert(insert_index + 1, self.diff_time_stmt)
                cur_node.insert(insert_index + 1, self.end_time_stmt)
...
class Rewrite_multiple_ass_insert_time_swap_vars(ast.NodeTransformer):
    def __init__(self,old_node,new_node):
        self.old_node =old_node
        self.new_node =new_node
        TimerStmt.__init__(self)
        self.old_node_ass_list=[]
        for e in self.old_node:
            for node in ast.walk(ast.parse(e)):
                if isinstance(node,(ast.Assign, ast.AnnAssign)):
                    self.old_node_ass_list.append(node)
                    break
    ...
    def remove_node(self, cur_node):
        ...
        for ind, child in enumerate(cur_node):
            ...
            if child.lineno==self.new_node.lineno and ast.unparse(child)==ast.unparse(self.new_node):
                ...
                insert_index = ind#cur_node.index(remove_list[-1])
                # insert_index=beg-len(remove_list)+1
                ...
                # insert_index = cur_node.index(remove_list[0])
                for e in self.old_node_ass_list[::-1]:
                    cur_node.insert(insert_index,e)
                cur_node.insert(insert_index, self.start_time_stmt)
                cur_node.insert(insert_index, self.import_time_stmt)
...
class Rewrite_boolop(ast.NodeTransformer):
    ...
    def visit_BoolOp(self, node):
        ...
        return node
def replace_content_multi_ass_simple(file_path,ass_list,new_code):
...
def replace_content_multi_ass(file_path,ass_list,new_code, me_name):
    content = util.load_file_path(file_path)
    all_tree = ast.parse(content)
    old_tree_str = ast.unparse(all_tree)
    rew = Rewrite_multiple_ass(ass_list, new_code)
    ...
    return content, ast.unparse(all_tree),flag_same
def replace_content_multi_ass_swap(file_path,ass_list,new_code, me_name):
    content = util.load_file_path(file_path)
    all_tree = ast.parse(content)
    old_tree_str = ast.unparse(all_tree)
    rew = Rewrite_multiple_ass_swap(ass_list, new_code)
    all_tree = rew.visit(all_tree)
    flag_same=old_tree_str == ast.unparse(all_tree)
    print("whether code1 same: ", flag_same)
    ana_py = ast_performance_util.Fun_Analyzer_Insert_Stmt(me_name)
...
def replace_content_multi_ass_insert_time(file_path, ass_list, new_code, me_name):
    ...
    all_tree = ana_py.visit(all_tree)
    return content, ast.unparse(all_tree), flag_same
def replace_content_multi_ass_insert_time_swap_vars(file_path, ass_list, new_code, me_name):
    content = util.load_file_path(file_path)
    all_tree = ast.parse(content)
    old_tree_str = ast.unparse(all_tree)
    rew = Rewrite_multiple_ass_insert_time_swap_vars(ass_list, new_code)
    all_tree = rew.visit(all_tree)
    flag_same = old_tree_str == ast.unparse(all_tree)
    print("whether code1 same: ", flag_same)
    ana_py = ast_performance_util.Fun_Analyzer_Insert_Stmt(me_name)
    all_tree = ana_py.visit(all_tree)
    return content, ast.unparse(all_tree), flag_same
def replace_content_multi_ass_insert_time_simple(file_path, ass_list, new_code):
    content = util.load_file_path(file_path)
    all_tree = ast.parse(content)
    old_tree_str = ast.unparse(all_tree)
    rew = Rewrite_multiple_ass_insert_time(ass_list, new_code)
