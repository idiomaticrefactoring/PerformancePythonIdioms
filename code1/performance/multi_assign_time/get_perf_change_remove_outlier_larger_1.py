...
if __name__ == '__main__':
    ...
    print("len: ", file_html_list_filter[0],len(file_html_list_filter), len(set(file_html_list_filter)))

    save_code_info_dir_add_performance_change=util.data_root_mv + "performance/a_multi_assign/a_multi_assign_iter_invoca_add_perf_change_new_remove_outlier_update_some/"
    ...
    for ind, file_name in enumerate(sorted(os.listdir(bench_time_info_dir)[:])):
        # print("file_name: ",file_name,file_name[:-4].split("_")[-1])
        # break
        if  file_name.endswith('.log'):
            print(file_name)
            continue
        # if file_name in file_html_list_filter.split("/")[-1]:
