...
def copy_data():
    bench_time_info_dir_2=util.data_root_mv+"performance/a_for_multi_tar_single/a_for_multi_tar_iter_invoca_2/"

    if not os.path.exists(bench_time_info_dir_2):
        os.mkdir(bench_time_info_dir_2)
    for ind, file_name in enumerate(sorted(os.listdir(bench_time_info_dir)[:])):
        ...
        for key_e in compli_code_time_dict[0]:
            if compli_code_time_dict[0][key_e]:
                shutil.copyfile(bench_time_info_dir + file_name, bench_time_info_dir_2 + file_name)
                print("yes, I do not need to configure once again")
                break
        else:
            need_configure_flag = 1
...
if __name__ == '__main__':
    ...
    bench_time_info_dir=util.data_root_mv+"performance/a_for_multi_tar_single/a_for_multi_tar_iter_invoca/"
    bench_time_info_dir=util.data_root_mv+"performance/a_for_multi_tar_single/a_for_multi_tar_iter_invoca_2/"
    bench_time_info_dir=util.data_root_mv+"performance/a_for_multi_tar_single_2/a_for_multi_tar_iter_invoca_2/"
    bench_time_info_dir = util.data_root_mv + "performance/a_for_multi_tar_single_3_new/a_for_multi_tar_iter_invoca_2/"


    save_code_info_dir_add_performance_change=util.data_root_mv + "performance/a_for_multi_tar_single/a_for_multi_tar_iter_invoca_add_perf_change_new/"
    ...
    csv_perf_change_dir=util.data_root + "performance/a_for_multi_tar/csv/"
    file_name_list=[]
    for ind, file_name in enumerate(sorted(os.listdir(bench_time_info_dir)[:])):
        # break
        # if "519" not in file_name:
        ...
        lab_code_info = util.load_pkl(bench_time_info_dir, file_name_no_suffix)
        lab_code_info: CodeInfo
        if lab_code_info.file_html!="https://github.com/influxdata/influxdb-python/tree/master/influxdb/resultset.py":
            continue
        '''
