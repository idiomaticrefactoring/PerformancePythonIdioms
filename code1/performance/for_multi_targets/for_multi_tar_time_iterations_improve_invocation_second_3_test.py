...
if __name__ == '__main__':
    ...
    for ind_code,code in enumerate(code_info_list[:]):
        # if ind_code+offset not in [65]:
        #     continue
        one_code_time_start = time.time()
        code:code_info.CodeInfo
        file_html=code.file_html
        if file_html!="https://github.com/influxdata/influxdb-python/tree/master/influxdb/resultset.py":
            continue
        ...
        print("len(all_invocations_time_list): ",len(all_invocations_time_list),all_invocations_time_list)#,flag_total_test_html,flag_total_test_html_instances)
        print("len(all_invocations_time_list_pythonic): ",len(all_invocations_time_list_pythonic),all_invocations_time_list_pythonic)#, flag_total_test_html_pythonic, flag_total_test_html_instances_pythonic)
