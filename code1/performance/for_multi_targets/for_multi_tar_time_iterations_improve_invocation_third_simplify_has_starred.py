...
def code_test_get_time(repo_path, old_path, old_content, test_me_inf_list, is_pythonic_timeflag=0, is_own_config=0,
    ...
    try:
        ...
        for test_html, each_rela_path, cl, me in test_me_inf_list:
            ...
            #         num_add_ele_list[(test_html, each_rela_path, cl, me)] = [get_num_add_ele_list[i]]
            #     else:
            #         num_add_ele_list[(test_html, each_rela_path, cl, me)].append(get_num_add_ele_list[i])

                # if invo==0:
                #     num_add_ele = performance_util.get_num_add_ele(run_test_result_new)
                #     num_add_ele_list.append(num_add_ele)
            print("get_time_list: ", get_time_list,len(get_time_list), len(get_time_list) % iterations)
...
if __name__ == '__main__':
    ...
    pro_time_start = time.time()
    save_code_info_dir = util.data_root_mv + "performance/a_for_multi_tar_single_2/a_for_multi_tar_iter_invoca_2/"
    error_code_dir = util.data_root_mv + "performance/a_for_multi_tar_single_2/erro_record_dir_2/"
    time_out_dir = util.data_root_mv + "performance/a_for_multi_tar_single_2/time_out_record_dir_2/"
    save_code_info_dir = util.data_root_mv + "performance/a_for_multi_tar_single_3_new/a_for_multi_tar_iter_invoca_2/"
    error_code_dir = util.data_root_mv + "performance/a_for_multi_tar_single_3_new/erro_record_dir_2/"
    time_out_dir = util.data_root_mv + "performance/a_for_multi_tar_single_3_new/time_out_record_dir_2/"
    file_html_dir=util.data_root_mv + "performance/a_for_multi_tar_single_2/"

    file_dir = util.data_root + "format_code_info/"
    save_code_info_dir_num = util.data_root_mv + "performance/a_for_multi_tar_single_2_unpack_ele_len_new/a_for_multi_tar_iter_invoca_2/"
    # save_code_info_dir = util.data_root_mv + "performance/a_for_multi_tar_single_2_unpack_ele_len_new/a_for_multi_tar_iter_invoca_2/"

    # file_html_dir = util.data_root_mv + "performance/a_call_star_single_2/"
    file_html_list = util.load_json(file_html_dir, "all_file_html_for_multi_tar")
    ...
    for ind_code,code in enumerate(code_info_list[:end]):
        ...
        try:
            lab_code_info_add_num_ele = util.load_pkl(save_code_info_dir_num, code_info_file_name)
        except:
            continue
        # lab_code_info_add_num_ele.num_ele_list
        # lab_code_info_add_num_ele = util.load_pkl(save_code_info_dir_num, num_ele_offset_file_name_no_suffix)
        len_star=0
        ...
        print(f">>>>>>>>>>>>>>>> total_name of {ind_code+offset} code: ", file_html,cl, me_name, total_name,code.own_config,len_star)
        flag_total_test_html = dict()
        flag_total_test_html_instances = dict()
        ...
        new_code = ast.unparse(new_code_list_simplify[0][1])
        target_str=ast.unparse(new_code_list_simplify[0][1].target)
        count_target=target_str.count(", ")-target_str.count("*")+1
        print("count_target: ",count_target,len_star)
        if count_target==len_star:
            import re

            new_code = re.sub(", \*[^\)]*\_len", "", new_code)
            ...
            for e_node in ast.walk(ast.parse(new_code)):
                if isinstance(e_node, ast.For):
                    new_code_list_simplify[0][1] = e_node
                    break
        ...
        # '''
        old_content, new_content, flag_same = performance_replace_content_by_ast_for_multi_targets.replace_content_var_unpack_for_target_insert_time_add_len_unpack(
        ...
        # print("num_add_ele_list: ", num_add_ele_list_test_one_iter,all_test_case_time_list_old)
        # code_info_file_name = total_name + "_" + str(ind_code + offset)
        ...
        for invo in range(invocations):
            ...
            all_test_case_time_list_old, all_valid_flag,dict_run_test, config_again_flag = code_test_get_time(repo_path, old_path,
                ...
                is_own_config=code.own_config,iterations=iterations, invo=invo,
                                                                             threshhold=threshold,
