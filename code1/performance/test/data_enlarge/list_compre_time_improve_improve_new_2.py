...
def code_test_get_time(repo_path,old_path,old_content,test_me_inf_list,is_pythonic_timeflag=0,is_own_config=0,
    ...
    # print(">>>>>>>>>>>>>>>>>>>>>>>> come here invoc ", is_pythonic_timeflag)
    # flag_total_test_html=dict()
    # flag_total_test_html_instances=dict()
        ...
        #     print(">>>>>>>come invoca ", invo,len(test_me_inf_list))
            ...
            for test_html, each_rela_path, cl, me in test_me_inf_list:
                ...
                if not is_own_config:
                    run_test_result_new = configure_pro_envir_util.run_test_file_capture_output_each_fun_Popen(new_test_html,
                                                                                                repo_path,
                                                                                                fun_list=fun_list,
                                                                                                export_python=True,prefine_cpus=prefine_cpus)
                else:
                    run_test_result_new = configure_pro_envir_util.run_test_file_capture_output_each_fun_Popen(new_test_html, repo_path,
                ...
                # print(">>>>>>>>>>>>>>>>>>>>>>>> come here invoc ",invo,is_pythonic_timeflag)
...
if __name__ == '__main__':
    ...
    for for_num in for_num_list:
        # pass
        save_code_info_dir=util.data_root+"performance/list_compre_benchmarks_new_enlarge_elements_"+str(for_num)+"/"

        # for_num=2
        for ind_code, code in enumerate(code_info_list[offset:88]):#
            # if ind_code>0:
            #     break
            code:code_info.CodeInfo
            file_html=code.file_html
            # if file_html!="https://github.com/quantumlib/OpenFermion/tree/master/src/openfermion/circuits/trotter_exp_to_qgates.py":#"https://github.com/kellyjonbrazil/jc/tree/master/jc/parsers/csv.py":#"https://github.com/kellyjonbrazil/jc/tree/master/jc/parsers/systemctl_luf.py":
            #     continue
            test_me_inf_list = code.test_case_info_list
            for_node, assign_node, remove_ass_flag, new_tree=code.code_info
            str_for_node="\n".join(["    "+e for e in ast.unparse(for_node).split('\n')])
            another_for=f"for i_zejun in range({for_num}):\n"+str_for_node
            for new_for in ast.walk(ast.parse(another_for)):
                if isinstance(new_for,ast.For):
                    for_new_node=new_for
                    break

            new_tree_code_list=ast.unparse(new_tree)[:-1].split("for ")
            new_tree_code_list[0]=new_tree_code_list[0]+f"for i_zejun in range({for_num}) "
            new_tree_code="for ".join(new_tree_code_list)+"]"
            for new_tree_child in ast.walk(ast.parse(new_tree_code)):
                if isinstance(new_tree_child,ast.Assign):
                    new_tree_new=new_tree_child
                    break
            ...
            for invo in range(invocations):
                ...
                all_test_case_time_list_old,all_valid_flag = code_test_get_time(repo_path, old_path,
                                                                 old_content,
                                                                 test_me_inf_list,is_pythonic_timeflag=0,iterations=iterations,invo=invo,threshhold=threshold,
                                                                 flag_total_test_html=flag_total_test_html,
                                                                 flag_total_test_html_instances=flag_total_test_html_instances,
                                                                 num_add_ele_list=num_add_ele_list,prefine_cpus=prefine_cpus)
