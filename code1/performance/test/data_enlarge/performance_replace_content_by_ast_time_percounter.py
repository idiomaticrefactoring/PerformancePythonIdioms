            ...
            for ind,child in enumerate(node.body):
                ...
                # for ind, child in enumerate(node.body):
                    ...
                    if child.lineno == self.old_node.lineno and isinstance(child, ast.For) and ast.unparse(
                        ...
                        node.body.remove(child)
                        var = ast.unparse(self.ass).split("=")[0]
                        nl='\n'

                        print_len_stmt = f"print('''\nlen: ''',len({var}))"#'print("len: ",'+f'''len({var}),end={nl})'''
                        for e in ast.walk(ast.parse(print_len_stmt)):
                            if isinstance(e, ast.stmt):
                                node.body.insert(ind + 5, e)
                                break
                        node.body.insert(ind + 5, self.print_total_stmt)
                        node.body.insert(ind+5, self.total_time_stmt)
                        node.body.insert(ind+5, self.end_time_stmt)
                        node.body.insert(ind+5, self.new_node)
                        node.body.insert(ind+5, self.start_time_stmt)
