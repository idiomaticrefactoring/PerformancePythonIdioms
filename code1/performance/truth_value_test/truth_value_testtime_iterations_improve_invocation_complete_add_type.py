...
import performance_replace_content_by_ast_time_percounter_add_type
...
def code_test_get_time(repo_path, old_path, old_content, test_me_inf_list, is_pythonic_timeflag=0, is_own_config=0,
                       ...
                       num_add_ele_list=[], new_content="",type_add_list=dict()):
    dict_run_test = dict()
    all_valid_flag = 1
    # flag_total_test_html=dict()
    # flag_total_test_html_instances=dict()
    ...
    try:
        ...
        for test_html, each_rela_path, cl, me in test_me_inf_list:
            ...
            if is_pythonic_timeflag:
                ...
                get_var_list = performance_util.get_var_two_dimen(run_test_result_new)
            else:
            ...
            total_var_type_list = [[] for i in range(test_case_num)]
            ...
            for i in range(test_case_num):
                for j in range(iterations):
                    ...
                    if invo == 0 and not is_pythonic_timeflag:
                        total_var_type_list[i].append(get_var_list[test_case_num * j + i])
            ...
            all_test_case_time_list[(test_html, each_rela_path, cl, me)] = total_time_list
            if invo == 0 and not is_pythonic_timeflag:
                type_add_list[(test_html, each_rela_path, cl, me)] = total_var_type_list
...
if __name__ == '__main__':
    ...
    save_code_info_dir=util.data_root_mv+"performance/a_truth_value_test/a_truth_value_test_iter_invoca_total_data_add_type/"
    save_code_info_dir_no_type=util.data_root_mv+"performance/a_truth_value_test/a_truth_value_test_iter_invoca_total_data/"

    error_code_dir = util.data_root_mv + "performance/a_truth_value_test/erro_record_dir/"
    time_out_dir=util.data_root_mv + "performance/a_truth_value_test/time_out_record_dir/"
    no_through_code_dir=util.data_root_mv + "performance/a_truth_value_test/no_through_code_dir/"
    file_dir=util.data_root+"format_code_info/"
    file_name="truth_value_test_complete"#"multi_ass_complete"
    code_time_list=[]
    case_is_true_list_filter = []
    time_out_list_filter=[]
    no_through_code_list=[]
    ...
    for ind_code,code in enumerate(code_info_list[original_offset:end]):

        # if ind_code not in [0]:
        #     continue
        #33是while,7，8，9，10,67,125是assert
        # if ind_code!=67:#108:
        #     continue
        # if ind_code<0:
        #     continue
        ...
        # if "while" not in ast.unparse(old_tree):
        #     continue
        # print("old_tree: ",old_tree.lineno,ast.unparse(old_tree),ast.unparse(new_tree))
        # continue
        ...
        if not os.path.exists(save_code_info_dir_no_type + code_info_file_name + ".pkl"):
            print("it does not need to save: ",save_code_info_dir_no_type + code_info_file_name + ".pkl")
            continue
        # save_code_info_dir, code_info_file_name
        if os.path.exists(save_code_info_dir + code_info_file_name + ".pkl"):
            print(">>>>>>>>file has existed: ", save_code_info_dir + code_info_file_name + ".pkl")
            continue
        ...
        # print(old_tree.lineno,ast.unparse(old_tree))
        old_content, new_content, flag_same = performance_replace_content_by_ast_time_percounter_add_type.replace_content_truth_value_test_insert_time(
            old_path, old_tree, new_tree, me_name)
        # print(old_content)
        # print("***********************")
        # print(new_content)
        if flag_same:
            print("there is no difference code  ", file_html, cl, me_name, total_name)
            case_is_true_list_filter.append([ind_code,ind_code+offset, "complicated code", file_html, cl, me_name, total_name, total_name + "_" + str(ind_code + offset)])
            continue
        # break

        # old_content, new_content, flag_same = performance_replace_content_by_ast_time_percounter.replace_content_truth_value_test(
        #     old_path, old_tree, new_tree, me_name)
        # print("***********************")
        # print(new_content)
        # if flag_same:
        ...
        all_test_case_time_list_old, all_valid_flag, dict_run_test_complicated = code_test_get_time(repo_path,
                                                                                                    ...
                                                                                                    prefine_cpus=prefine_cpus,
                                                                                                    new_content=new_content,type_add_list=type_add_list)
        ...
        if one_invo_end - one_invo_start > 10:
            ...
            time_out_list_filter.append(
                [ind_code, ind_code + offset, "complicated code", file_html, cl, me_name, total_name, total_name + "_" + str(ind_code + offset)])
            continue
        ...
        for key in all_test_case_time_list_old:
            ...
            print(">>>>>>>>>>>>>>>It does not through the truth value test")
            no_through_code_list.append([ind_code, ind_code + offset, "complicated code", file_html, cl, me_name, total_name, total_name + "_" + str(ind_code + offset)])
            continue
        #break
        '''
        ...
        if flag_same:
            print("there is no difference code  ", file_html, cl, me_name, total_name)
            case_is_true_list_filter.append([ind_code, "pythonic code", file_html, cl, me_name, total_name, total_name + "_" + str(ind_code + offset)])
            continue
        ...
        '''
        code.type_add_list = type_add_list
        code.compli_code_time_dict = all_test_case_time_list_old
        print(f"******************one code pair summary, the index is {ind_code}************************************")
        print("len(all_invocations_time_list): ",len(all_test_case_time_list_old))#,flag_total_test_html,flag_total_test_html_instances)
        print("len(all_invocations_time_list_pythonic): ",len(type_add_list))#, flag_total_test_html_pythonic, flag_total_test_html_instances_pythonic)
        print("num_add_ele_list: ",type_add_list)
        
        code_info_file_name = total_name + "_" + str(ind_code + offset)
        
        util.save_pkl(save_code_info_dir, code_info_file_name, code)
        # lab_code_info = util.load_pkl(save_code_info_dir, code_info_file_name)
        # print(lab_code_info.type_add_list)
    ...
    util.save_pkl(error_code_dir, "error_code_is_true_info_complete_1_add_type", case_is_true_list_filter)
    util.save_pkl(time_out_dir, "time_out_code_info_complete_1_add_type", time_out_list_filter)
    util.save_pkl(no_through_code_dir, "no_through_code_info_complete_1_add_type", no_through_code_list)


    # util.save_pkl(no_through_code_dir, "no_through_code_info_2", no_through_code_list)
    # print("no_through_code_list: ",no_through_code_list)
