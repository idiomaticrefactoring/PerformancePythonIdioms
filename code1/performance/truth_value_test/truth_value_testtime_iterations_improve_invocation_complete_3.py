...
if __name__ == '__main__':
    ...
    # 521的时候，出现内存错误看truth_value_testtime_iterations_improve_invocation_complete_3_add_next.log
    for ind_code,code in enumerate(code_info_list[original_offset:]):
        ...
        try:
            ...
            traceback.print_exc()
            print(">>>>>>>>>>>>>save failed! ", save_code_info_dir + code_info_file_name)

        
    current_time=time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    util.save_pkl(error_code_dir, "error_code_is_true_info_add_some_second", case_is_true_list_filter)
    util.save_pkl(time_out_dir, "time_out_code_info_add_some_second", time_out_list_filter)
    util.save_pkl(no_through_code_dir, "no_through_code_info_add_some_second", no_through_code_list)
