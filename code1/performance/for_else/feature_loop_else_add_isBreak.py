...
def get_features_add_interval(save_code_info_dir_add_performance_change,save_code_info_dir_add_performance_change_remove_outlier):
    ...
    }# "branch_through_break": [], , "num_ele": [], "code_str": [],
    total_break=0
    ...
    for file_name in sorted(os.listdir(save_code_info_dir_add_performance_change)):
        ...
        for ind_tm, test_me in enumerate(perf_info_dict):
            ...
            for instance in perf_info_dict[test_me]:
                ...
                if test_me in lab_code_info_add_num_ele.num_ele_list and test_me in lab_code_info_add_break.num_ele_list:
                    if instance < len(lab_code_info_add_num_ele.num_ele_list[test_me]):# and instance < len(lab_code_info_add_break.num_ele_list[test_me]):
                        ...
                        break_list=lab_code_info_add_break.num_ele_list[test_me][instance]
                        break_list=[int(i_b) for i_b in break_list]
                        # print("num: ",num,break_list)
                        total_break+=sum(break_list)
                        total_code_instance+=len(break_list)
                        dict_pd['is_break'].append(1 if sum(break_list)!=len(break_list) else 0)

                        dict_pd['size_data'].append(int(num[0]))
                    else:
                        print(">>>>>>Wrong does not have ele: ",instance, file_html,file_name,len(lab_code_info_add_num_ele.num_ele_list[test_me]),len(lab_code_info_add_num_ele.num_ele_list[test_me][0]),len(lab_code_info_add_break.num_ele_list[test_me]),len(lab_code_info_add_break.num_ele_list[test_me][0]))
                        continue
                else:
...
if __name__ == '__main__':
    ...
    csv_feature_file_name_corr= csv_perf_change_dir+"train_data_a_for_else_add_size_add_isBreak.csv"
