...
import performance_replace_content_by_ast_for_else
...
def code_test_get_time(repo_path, old_path, old_content, test_me_inf_list, is_pythonic_timeflag=0, is_own_config=0,
    ...
    try:
        ...
        for test_html, each_rela_path, cl, me in test_me_inf_list:
            ...
            print(f">>>>>>>>>>{(test_html, each_rela_path, cl, me)} {fun_list} run_test_result_new:\n ",run_test_result_new)#,run_test_result_new)#,run_test_result_new)  # , run_test_result_new)
            ...
            get_num_add_ele_list = performance_util.get_isBreak_two_dimen(run_test_result_new)
...
if __name__ == '__main__':
    ...
    save_code_info_dir = util.data_root_mv + "performance/a_for_else/a_for_else_tar_iter_invoca_2_once_again/"
    error_code_dir = util.data_root_mv + "performance/a_for_else/erro_record_dir_2_once_again/"
    time_out_dir = util.data_root_mv + "performance/a_for_else/time_out_record_dir_2_once_again/"
    no_time_dir = util.data_root_mv + "performance/a_for_else/no_time_record_dir_2_once_again/"
    save_code_info_dir = util.data_root_mv + "performance/a_for_else_add_isBreak/a_for_else_tar_iter_invoca_2_once_again/"
    error_code_dir = util.data_root_mv + "performance/a_for_else_add_isBreak/erro_record_dir_2_once_again/"
    time_out_dir = util.data_root_mv + "performance/a_for_else_add_isBreak/time_out_record_dir_2_once_again/"
    no_time_dir = util.data_root_mv + "performance/a_for_else_add_isBreak/no_time_record_dir_2_once_again/"

    file_dir = util.data_root + "format_code_info/"
    file_name = "for_else_complete"  # "multi_ass_sample_317"#"multi_ass_complete"
    code_time_list = []
    ...
    # threshold = 1#10
    # iterations = 2#15
    prefine_cpus = " "#"taskset -c 2,3  "#"taskset -c 2,3 "
    offset = 0
    end = 19
    for ind_code, code in enumerate(code_info_list[:]):
        # if ind_code<3:
        #     continue
        # if ind_code not in [5]:
        #     continue
        one_code_time_start = time.time()
        code: code_info.CodeInfo
        file_html = code.file_html
        # if file_html!="https://github.com/indigo-dc/udocker/tree/master/udocker/docker.py":#"https://github.com/kellyjonbrazil/jc/tree/master/jc/parsers/csv.py":
        #     continue
        # if file_html!="https://github.com/quantumlib/OpenFermion/tree/master/src/openfermion/measurements/fermion_partitioning.py":
        #     continue
        test_me_inf_list = code.test_case_info_list
        old_tree, new_tree, break_list_in_for, child, child_copy, \
        ass_init, if_varnode, init_ass_remove_flag = code.code_info
        ...
        print(child.lineno,">>>1:\n ",ast.unparse(child),">>>2:\n",ast.unparse(child_copy),ast.unparse(if_varnode))
        # old_content, new_content, flag_same = performance_replace_content_by_ast_for_else.replace_content_for_else_insert_time_add_size(
        #     old_path, child, child_copy, ass_init, if_varnode, init_ass_remove_flag, me_name)

        old_content, new_content, flag_same = performance_replace_content_by_ast_for_else.replace_content_for_else_add_isBreak(
            old_path, child, child_copy, ass_init, if_varnode, init_ass_remove_flag, me_name)
        ...
        # print(new_content)
        # break
        # old_content, new_content, flag_same = performance_replace_content_by_ast_for_else.replace_content_for_else(
        # old_path, child, child_copy, ass_init, if_varnode, init_ass_remove_flag, me_name)
        # print(old_content)
        # print("***********************")
        # print(new_content)
        ...
        while 1:
            ...
            all_test_case_time_list_old, all_valid_flag, dict_run_test_complicated,config_again_flag = code_test_get_time(repo_path,
                                                                                                        old_path,
                                                                                                        old_content,
                                                                                                        test_me_inf_list,
        ...
        # break
        for test_case in all_test_case_time_list_old:
            if all_test_case_time_list_old[test_case]:
                break
        else:
            print("there is no time ", one_invo_end - one_invo_start)
            no_time_list.append([ind_code, ind_code + offset, file_html, cl, me_name, total_name, test_me_inf_list])
        ...
        # util.save_pkl(save_code_info_dir, code_info_file_name, code)
        # if time_out_flag:
        #     time_out_list_filter.append([ind_code,ind_code+offset, "complicated code", file_html, cl, me_name, total_name])
        #     continue
        ...
        for invo in range(invocations):
            ...
            all_test_case_time_list_old, all_valid_flag, dict_run_test,config_again_flag = code_test_get_time(repo_path, old_path,
                                                                                            old_content,
                                                                                            test_me_inf_list,
                                                                                            is_pythonic_timeflag=1,is_own_config=code.own_config,
                                                                                            iterations=iterations,
                                                                                            invo=invo,
                                                                                            threshhold=threshold,
        ...
        print("len(all_invocations_time_list): ",
              len(all_invocations_time_list))  # ,flag_total_test_html,flag_total_test_html_instances)
        print("len(all_invocations_time_list_pythonic): ",
...
'''
    ...
    # util.save_pkl(no_time_dir, "no_time_code_info", no_time_list)
